...
По факту базовый HTTP-запуск остается модульным монолитом:
...
Для выделения микросервисов добавлен второй транспортный слой - gRPC:

- `cmd/auth-service` поднимает `cityhawk.auth.v1.AuthService`;
- `cmd/profile-service` поднимает `cityhawk.profile.v1.ProfileService`;
- `cmd/events-service` поднимает `cityhawk.events.v1.EventsService`;
- `cmd/social-service` поднимает `cityhawk.social.v1.SocialService`;
- `cmd/support-service` поднимает `cityhawk.support.v1.SupportService`.

Контракты лежат в `proto/cityhawk/*/v1`, сгенерированные клиенты и серверные интерфейсы - в `pkg/pb/*/v1`, а реальные реализации серверов - в `internal/*/delivery/grpc`.
...
Для микросервисного режима используется тот же принцип: зависимости собираются в `internal/app/grpc_services.go`, а каждый домен получает свой процесс и свой gRPC-контракт.
...
Правило разделения для gRPC такое же, как в материалах по микросервисам: внешний сервис видит только protobuf-контракт, а детали хранения и реализации остаются внутри процесса. Например, `AuthService.ValidateAccessToken` дает другим сервисам единую точку проверки access token, не раскрывая JWT/refresh/session storage.
