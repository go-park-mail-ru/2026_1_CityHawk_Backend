...
type googleRoundTrip func(*http.Request) (*http.Response, error)

func (f googleRoundTrip) RoundTrip(req *http.Request) (*http.Response, error) { return f(req) }
...
func TestNewOAuthConfigAndFetchProfile(t *testing.T) {
	...
	if err != nil || cfg.ClientID != "id" {
		t.Fatalf("NewOAuthConfig() error = %v cfg=%+v", err, cfg)
	}

	original := http.DefaultClient
	http.DefaultClient = &http.Client{Transport: googleRoundTrip(func(req *http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: http.StatusOK,
			Body:       io.NopCloser(strings.NewReader(`{"id":"42","email":"user@example.com","name":"User"}`)),
...
func TestGatewayFetchIdentityAndProfileErrors(t *testing.T) {
	...
	identity := authmodel.OAuthIdentity{Provider: "google"}
	if identity.Provider != "google" {
		t.Fatal("unexpected identity struct behavior")
	}
}
