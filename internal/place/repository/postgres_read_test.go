...
func TestPostgresRepositoryReadMethodsWithFakeDB(t *testing.T) {
	...
	db := &fakePlaceDB{
		rows: map[string][][]any{
			...
			"FROM candidates":                                           {{"event-1", "event", "Jazz night"}},
			"FROM filtered_events filtered":                             {{"event-1", "Jazz night", "Short", "/uploads/event.png", []string{"tag-1"}, []string{"Jazz"}, &now, "Hall", "Lenina 1", 1, true, 3}},
